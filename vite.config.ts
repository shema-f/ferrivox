...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    build: {
      ...
      // Vite 8 builds with Rolldown. `codeSplitting.groups` (the supported
      // replacement for webpack-style `manualChunks`) keeps heavy vendors in
      // separate, long-cacheable chunks: three.js/R3F ships in its own chunk
      // (and is only fetched when the lazy-loaded Scene3D mounts), while the
      // React runtime and Supabase client stay in small, stable chunks.
      rolldownOptions: {
        output: {
          codeSplitting: {
            groups: [
              {
                name: "vendor-three",
                test: /node_modules[\\/](three|@react-three|postprocessing|gsap)[\\/]/,
                priority: 2,
              },
              {
                name: "vendor-react",
...
/**
 * Precompresses build output as both gzip (`.gz`) and Brotli (`.br`) at
 * build time, so static hosts and nginx can serve pre-encoded responses
 * (`gzip_static on` / `brotli_static on`) instead of compressing on the
 * fly — lower TTFB, no runtime CPU cost.
 *
 * - Brotli level 11 is fine here: it runs once at build time.
 * - Only compresses formats that actually benefit (JS/CSS/SVG/HTML/JSON/
 *   text). Already-compressed formats (images/fonts) are skipped.
 * - Threshold: files under 1 KiB are skipped (not worth the extra request).
 */
...
function precompressAssets(): Plugin {
  return {
    ...
    async writeBundle(options, bundle) {
      const outDir =
        "dir" in options && typeof options.dir === "string"
          ? options.dir
      ...
      for (const file of Object.keys(bundle)) {
        ...
        await Promise.all([
          fsp.writeFile(filePath + ".gz", gz),
          fsp.writeFile(filePath + ".br", br),
        ])
      ...
      if (gzipTotal > 0 || brotliTotal > 0) {
        const kb = (n: number) => (n / 1024).toFixed(0) + " kB"

        console.log(
          `[precompress] wrote ${kb(gzipTotal)} gzip + ${kb(brotliTotal)} brotli sidecars`,
        )
      }

      // Sanity: the entry HTML must have sidecars too (hosts serve it pre-encoded)
      const indexPath = path.join(outDir, "index.html")

      if (fs.existsSync(indexPath) && !fs.existsSync(indexPath + ".br")) {
        const source = await fsp.readFile(indexPath)

        if (source.length >= MIN_COMPRESS_BYTES) {
          const [gz, br] = await Promise.all([
            gzipAsync(source, { level: 9 }),
            brotliAsync(source, { params: { [zlib.constants.BROTLI_PARAM_QUALITY]: 11 } }),
          ])

          await Promise.all([
            fsp.writeFile(indexPath + ".gz", gz),
            fsp.writeFile(indexPath + ".br", br),
          ])
        }
      }
    },
